...
public abstract strictfp class AbstractWorkspace
  ...
  public abstract scala.collection.immutable.ListMap<String, ProcedureInterface> procedures();
  public abstract void setProcedures(scala.collection.immutable.ListMap<String, ProcedureInterface> procedures);
  ...
  public WeakHashMap<String, ProcedureInterface> codeBits =
      new WeakHashMap<String, ProcedureInterface>();

  public ProcedureInterface compileForRun(String source, org.nlogo.nvm.Context context,
                                 boolean reporter)
      throws CompilerException {
    String key = source + "@" + context.activation.procedure().args().size() +
        "@" + context.agentBit;
    scala.Option<ProcedureInterface> storedProc = codeBits.get(key);
    if (storedProc.isEmpty()) {
      ProcedureInterface proc = evaluator.compileForRun(source, context, reporter);
