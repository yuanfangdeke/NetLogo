...
// ought to be in the api package, except oops, it depends on nvm.ProcedureInterface - ST 2/23/09

trait CompilerInterface {
  import ProcedureInterface.ProceduresMap
...
trait AuxiliaryCompilerInterface {
  ...
  def isReporter(s: String, program: Program, procedures: ListMap[String, ProcedureInterface], extensionManager: ApiExtensionManager, compilationEnv: CompilationEnvironment): Boolean
  ...
  @throws(classOf[CompilerException])
  def checkCommandSyntax(source: String, program: Program, procedures: ListMap[String, ProcedureInterface],
                         extensionManager: ApiExtensionManager, parse: Boolean, compilationEnv: CompilationEnvironment)

  @throws(classOf[CompilerException])
  def checkReporterSyntax(source: String, program: Program, procedures: ListMap[String, ProcedureInterface],
                          extensionManager: ApiExtensionManager, parse: Boolean, compilationEnv: CompilationEnvironment)
}
